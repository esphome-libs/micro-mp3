...
#
# Builds fuzz_mp3_decode with clang's source-based coverage instrumentation,
...
if [[ ! -d "$corpus" ]]; then
    ...
    echo "       run ./generate_seeds.sh and copy seeds_mp3/ -> $corpus/" >&2
    exit 1
fi
...
echo "[cov] building fuzz_mp3_decode"
cmake --build build-cov --target fuzz_mp3_decode >/dev/null

binary="build-cov/fuzz_mp3_decode"
